...
namespace AriDEVParser.Parsing.Parsers
{
    public static class ObjectHandler
    {
        ...
        public static MovementInfo ReadMovementUpdateBlock(Packet packet, Guid guid)
        {
            ...
            if (flags.HasFlag(UpdateFlag.AnimKits))
            {
        ...
        public static void HandleCompressedUpdateObject(Packet packet, LinkedList<PacketData> data)
        {
            var decompCount = packet.ReadInt32();
            var pkt = packet.Inflate(decompCount);
            HandleUpdateObject(pkt, data);
